...
async function getMerits(db, user, opts = {}) {
	...
	const bal = await db.collection(COLLECTIONS.BALANCES).findOne({ user: u });
...
function defaultContests(nowMs) {
	...
	// Presentation copy (Trello #182) shared by every client. Recognition/goal
	// framing only — no wagering (house rule). `art` maps to a client banner.
	return [
