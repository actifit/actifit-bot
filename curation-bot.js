...
function sendComment(post, vote_weight) {
	...
	return new Promise((resolve, reject) => {
		...
		if (content && content != '') {
			...
			var jsonMetadata = { tags: ['actifit'], app: 'actifit/v'+version, user_rank: post.user_rank, content_score: post.content_score, media_score: post.media_score, upvote_score: post.upvote_score, comment_score: post.comment_score, user_rank_score: post.user_rank_score, moderator_score: post.moderator_score, post_activity_score: post.activity_score, afit_tokens: token_count, post_upvote: vote_weight };
			if (!config.testing){
