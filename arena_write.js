...
// §7.4 tier policy lives in arena_tier.js — the SAME gate the ingest indexer
// (arena.indexArenaOp) applies, so a "valid" prediction here can't drift from
// what actually indexes. Re-exported below for backward compatibility.
const tierPolicy = require('./arena_tier');
const { TIER_MAY_CREATE, FRIENDLY_REWARD_KEYS, OFFICIAL_ONLY_OPS } = tierPolicy;
...
function validateProposedOp(op, opts = {}) {
	...
	// permissive than friendly) — done inside createTierErrors.
	const callerTier = tierPolicy.normalizeTier(opts.callerTier || 'friendly');
	...
	// The §7.4 origin-tier / official-only / friendly-reward gate — the SAME pure
	// function the ingest indexer applies to the op's real signer.
	errs.push(...tierPolicy.createTierErrors(op, callerTier));
